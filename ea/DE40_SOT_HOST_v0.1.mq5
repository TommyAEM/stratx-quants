//+------------------------------------------------------------------+
//| DE40_SOT_HOST_v0.1.mq5                                            |
//| Fork of DE40_X1_v2.20_PARENT: host shell + Module S (SOT v4.0).   |
//|                                                                    |
//| Module S = faithful MQL5 port of TV-verified SOT v4.0 GER40       |
//| (OANDA feed, 51tr/74.51%/PF3.491/DD3.12%):                        |
//|   - B: Imbalance (FVG) pullback entry                              |
//|   - D: Break & Retest entry                                        |
//|   - Trend gate: MA Intensity Percent Gap (EMA5/18 open, Goldilocks|
//|     [0.026,0.66] x23 bars, EMA7 smoothed) on M15                   |
//|   - Sessions (UTC): 1515-1530 / 1000-1015 / 1830-1845 / 1045-1100  |
//|   - NATR(17) percentile [5,95] over 200 bars; H4 EMA21 slope conf  |
//|   - Risk: SL=ATR(6)x1.7, TP fixed RR 1.0, hard true-RR (no BE)     |
//|                                                                    |
//| Modules A-H retained for regression (bits 0..6, 8); Module S =     |
//| bit 7 (value 128). Default mask = S only. Magic: S/FBO=4000,       |
//| GLK=4001, VWAP=4002.                                               |
//+------------------------------------------------------------------+
#property copyright "StratX Research"
#property version   "1.00"
#property strict
#property description "DE40 SOT Host v0.1 — Module S (SOT v4.0 GER40) + A-H regression modules"
#property description "SOT: TV-verified 51tr/74.51%/PF3.491/DD3.12% (OANDA GER40, M15)"
...
input int    InpModuleMask     = 128;    // 0=legacy; bits 0..5=FBO A-F, 6=GLK, 7=SOT, 8=VWAP (default S=128)
...
//=== MODULE S: SOT v4.0 GER40 (TV-verified) ===
input group "=== Module S: SOT v4.0 ==="
input bool   InpSotUseB        = true;   // B: Imbalance (FVG) pullback entry
input bool   InpSotUseD        = true;   // D: Break & Retest entry
input bool   InpSotAllowShort  = true;   // allow short entries (SOT long+short)
input ENUM_TIMEFRAMES InpSotTimeframe = PERIOD_M15; // SOT setup/eval timeframe
input double InpSotGldLowerPct = 0.026; // Goldilocks lower band (%)
input double InpSotGldUpperPct = 0.66;  // Goldilocks upper band (%)
input int    InpSotGldDuration = 23;    // bars in-band on same TF
input int    InpSotFastLen     = 5;     // fast EMA length
input int    InpSotSlowLen     = 18;    // slow EMA length
input int    InpSotSmoothLen   = 7;     // intensity smoothing length (EMA)
input double InpSotMinGapATR   = 0.0;   // min FVG gap (ATR) — UNUSED (reference fvg.py has no min-gap)
input int    InpSotFvgMaxAge   = 14;    // max FVG bars active (staleness)
input bool   InpSotFvgTouch    = true;  // FVG touch entry (vs zone entry)
input bool   InpSotFvgMitGuard = true;  // FVG mitigation guard
input int    InpSotDMaxBarsAfterBreak = 8; // D: max bars after break
input double InpSotDRetestTolATR = 2.25;// D: retest tolerance (ATR mult)
input bool   InpSotDImpFilter  = true;  // D: impulse filter
input double InpSotImpBodyMult = 1.3;   // impulse RECENCY body multiplier (vs sma(body,14))
input double InpSotDispMult    = 1.5;   // FVG zone-creation impulse x avgBody15 (config "Impulse Size x Avg Body")
input int    InpSotAvgBodyLen  = 15;    // average body length
input int    InpSotATRLen      = 6;     // SL ATR length
input double InpSotSL_ATRMult  = 1.7;   // SL = ATR x this
input double InpSotTP_RR       = 1.0;   // TP fixed RR
input bool   InpSotHardRR      = true;  // hard true-RR (no BE)
input bool   InpSotNatrGuard   = true;  // NATR regime guard
input int    InpSotNatrLen     = 17;    // NATR length
input int    InpSotNatrLookback= 200;   // NATR percentile lookback
input double InpSotNatrLowPct  = 5.0;   // NATR low percentile
input double InpSotNatrHighPct = 95.0;  // NATR high percentile
input bool   InpSotUseMain     = true;  // main window 15:15-15:30 UTC
input bool   InpSotUseAsia     = true;  // Asia open 10:00-10:15 UTC
input bool   InpSotUseLdn      = true;  // London open 18:30-18:45 UTC
input bool   InpSotUseNY       = true;  // NY open 10:45-11:00 UTC
input bool   InpSotNewsAvoid   = true;  // news avoidance 12:30-14:00 UTC
input bool   InpSotUseConf     = false; // H4 EMA21 confluence: OFF for parity (GER40 confluence transparent per verified config)
input int    InpSotConfLen     = 21;    // confluence EMA length
input int    InpSotConfSlopeBars = 5;   // confluence slope lookback (H4 bars)
...
#define SOT_MODULE_BIT  7          // Module S = bit 7 (value 128)
#define VWAP_MODULE_BIT 8          // Module H relocated to bit 8 (value 256)
...
//=== MODULE S (SOT) STATE ===
int      g_hSotATR    = INVALID_HANDLE;  // ATR(InpSotATRLen) on InpSotTimeframe
int      g_hSotFast   = INVALID_HANDLE;  // EMA(InpSotFastLen) on open
int      g_hSotSlow   = INVALID_HANDLE;  // EMA(InpSotSlowLen) on open
int      g_hSotNatrATR= INVALID_HANDLE;  // ATR(InpSotNatrLen) on InpSotTimeframe
int      g_hSotConf   = INVALID_HANDLE;  // EMA(InpSotConfLen) on PERIOD_H4

// Per-trade CSV capture + true-RR state
bool     g_sotPosition = false;
double   g_maeR        = 0;
...
int OnInit()
{
   ...
   if(ModOn(SOT_MODULE_BIT))
   {
      g_hSotATR     = iATR(g_activeSymbol, InpSotTimeframe, InpSotATRLen);
      g_hSotFast    = iMA(g_activeSymbol, InpSotTimeframe, InpSotFastLen, 0, MODE_EMA, PRICE_OPEN);
      g_hSotSlow    = iMA(g_activeSymbol, InpSotTimeframe, InpSotSlowLen, 0, MODE_EMA, PRICE_OPEN);
      g_hSotNatrATR = iATR(g_activeSymbol, InpSotTimeframe, InpSotNatrLen);
      if(g_hSotATR == INVALID_HANDLE || g_hSotFast == INVALID_HANDLE ||
         g_hSotSlow == INVALID_HANDLE || g_hSotNatrATR == INVALID_HANDLE)
      { Print("DE40: FATAL - SOT indicator handles failed"); return INIT_FAILED; }
      if(InpSotUseConf)
      {
         g_hSotConf = iMA(g_activeSymbol, PERIOD_H4, InpSotConfLen, 0, MODE_EMA, PRICE_CLOSE);
         if(g_hSotConf == INVALID_HANDLE)
         { Print("DE40: FATAL - SOT H4 confluence handle failed"); return INIT_FAILED; }
      }
   }
   ...
   Print("Goldilocks: ", ModOn(6) ? "ON" : "off", " | VWAP: ", ModOn(VWAP_MODULE_BIT) ? "ON" : "off");
   ...
   Print("SOT(v4.0): ", ModOn(SOT_MODULE_BIT) ? "ON" : "off",
         " | B=", InpSotUseB ? 1 : 0, " D=", InpSotUseD ? 1 : 0,
         " | SL=", DoubleToString(InpSotSL_ATRMult,2), "xATR(",
         InpSotATRLen, ") TP=", DoubleToString(InpSotTP_RR,2), "R",
         " | conf=", InpSotUseConf ? "H4EMA21" : "off");
   LogSymbolSpecs();
...
void OnDeinit(const int reason)
{
   ...
   if(g_hSotATR     != INVALID_HANDLE) IndicatorRelease(g_hSotATR);
   if(g_hSotFast    != INVALID_HANDLE) IndicatorRelease(g_hSotFast);
   if(g_hSotSlow    != INVALID_HANDLE) IndicatorRelease(g_hSotSlow);
   if(g_hSotNatrATR != INVALID_HANDLE) IndicatorRelease(g_hSotNatrATR);
   if(g_hSotConf    != INVALID_HANDLE) IndicatorRelease(g_hSotConf);
   DashCleanup();
}
...
void ManagePosition()
{
   ...
   // Module S: hard true-RR — fixed SL/TP only, no FBL exit management
   if(g_sotPosition) return;
...
//|   1 = Frankfurt 07-08, 2 = London 08-11, 4 = US overlap 13:30-16, |
//|   0 = out-of-hours                                                |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Capture per-trade metadata at entry (consumed at close by CSV)    |
//+------------------------------------------------------------------+
void CaptureTradeMeta(string module, string comment, double sl, double tp)
{
   g_csvModule   = module;
   g_csvComment  = comment;
   g_csvSL       = sl;
   g_csvTP       = tp;
   g_csvOpenTime = TimeCurrent();
   g_csvSide     = (g_tradeDir == 1) ? 1 : -1;
   MqlDateTime dt; TimeToStruct(TimeCurrent(), dt);
   g_csvWeekday = dt.day_of_week;
   int gh = dt.hour - InpServerUTC;
   if(gh < 0) gh += 24;
   if(gh >= 24) gh -= 24;
   g_csvGmtHour = gh;
   g_csvSession = SessionBucket(gh * 60 + dt.min);
}
...
void WriteTradeLog(datetime closeTime, double exitPrice, double R)
{
   ...
   FileWrite(handle,
             TimeToString(g_csvOpenTime, TIME_DATE | TIME_SECONDS),
             TimeToString(closeTime, TIME_DATE | TIME_SECONDS),
             (g_csvSide == 1) ? "BUY" : "SELL",
             DoubleToString(g_entryPrice, _Digits),
             DoubleToString(g_csvSL, _Digits),
             DoubleToString(g_csvTP, _Digits),
             DoubleToString(exitPrice, _Digits),
             DoubleToString(R, 4),
             DoubleToString(g_mfeR, 4),
             DoubleToString(g_maeR, 4),
             g_csvModule,
             IntegerToString(g_csvWeekday),
             IntegerToString(g_csvGmtHour),
             IntegerToString(g_csvSession),
             g_csvComment);
   FileClose(handle);
}
...
void OnTick()
{
   ...
   // Module S runs independently (own session/day/news/NATR gates).
   if(!g_inPosition && g_tradesToday < InpMaxTradesDay && ModOn(SOT_MODULE_BIT))
      SotEngine();
   ...
   if(!g_inPosition && g_tradesToday < InpMaxTradesDay && ModOn(6))
      GlkEngine(atr);
   if(!g_inPosition && g_tradesToday < InpMaxTradesDay && ModOn(VWAP_MODULE_BIT))
      VwapEngine(atr);
}
...
bool PlaceModuleEntry(int dir, double sl, double rr, string comment, long magic)
{
   ...
   g_minProfitR = 0;
   g_sotPosition = false;
   string modS = (magic == g_magicGLK) ? "GLK" : (magic == g_magicVWAP) ? "VWAP" : "MOD";
   CaptureTradeMeta(modS, comment, sl, tp);
...
//+------------------------------------------------------------------+
//| MODULE S: SOT v4.0 GER40 (TV-verified) M15 trend + trigger engine |
//| Faithful MQL5 port of verified_configs/GER40.json (OANDA feed).    |
//+------------------------------------------------------------------+
double SotGetATR()
{
   double buf[];
   ArraySetAsSeries(buf, true);
   if(CopyBuffer(g_hSotATR, 0, 0, 3, buf) < 3) return 0;
   return buf[1];
}

int SotGmtDOW()
{
   datetime gmt = TimeCurrent() - InpServerUTC * 3600;
   MqlDateTime dt; TimeToStruct(gmt, dt);
   return dt.day_of_week;
}

bool SotDayPass(int dow)
{
   if(dow == 2) return false;   // Tuesday OFF
   if(dow == 6) return false;   // Saturday OFF
   return true;
}

bool SotInWindow(int gmtMin, int h1, int m1, int h2, int m2)
{
   int lo = h1 * 60 + m1;
   int hi = h2 * 60 + m2;
   return (gmtMin >= lo && gmtMin < hi);
}

bool SotSessionPass(int gmtMin)
{
   if(InpSotUseMain && SotInWindow(gmtMin, 15, 15, 15, 30)) return true;
   if(InpSotUseAsia && SotInWindow(gmtMin, 10,  0, 10, 15)) return true;
   if(InpSotUseLdn  && SotInWindow(gmtMin, 18, 30, 18, 45)) return true;
   if(InpSotUseNY   && SotInWindow(gmtMin, 10, 45, 11,  0)) return true;
   return false;
}

bool SotNewsPass(int gmtMin)
{
   if(InpSotNewsAvoid && gmtMin >= 12 * 60 + 30 && gmtMin < 14 * 60) return false;
   return true;
}

//+------------------------------------------------------------------+
//| Trend gate: MA Intensity Percent Gap (EMA5/18 on open, Goldilocks |
//| band [lower,upper]%, duration bars, EMA-smoothed magnitude).       |
//| Returns +1 (up), -1 (down), 0 (no trend).                          |
//+------------------------------------------------------------------+
int SotTrendDir()
{
   int n = InpSotGldDuration + InpSotSmoothLen + 10;
   double f[], s[];
   ArraySetAsSeries(f, true);
   ArraySetAsSeries(s, true);
   if(CopyBuffer(g_hSotFast, 0, 1, n, f) < n) return 0;
   if(CopyBuffer(g_hSotSlow, 0, 1, n, s) < n) return 0;
   ...
   {
      if(s[i] == 0) return 0;
      raw[i] = MathAbs((f[i] - s[i]) / s[i] * 100.0);
   }

   double sm[];
   ArrayResize(sm, n);
   double k = 2.0 / (InpSotSmoothLen + 1.0);
   double e = raw[n - 1];
   sm[n - 1] = e;
   for(int i = n - 2; i >= 0; i--)
   {
   ...
   for(int i = 0; i < InpSotGldDuration; i++)
   {
      if(sm[i] < InpSotGldLowerPct || sm[i] > InpSotGldUpperPct) return 0;
   }
   if(f[0] > s[0]) return 1;
   if(f[0] < s[0]) return -1;
   return 0;
...
//+------------------------------------------------------------------+
//| NATR(17) percentile within [low,high] over lookback bars          |
//+------------------------------------------------------------------+
bool SotNatrPass()
{
   int n = InpSotNatrLookback;
   double a[], c[];
   ArraySetAsSeries(a, true);
   ArraySetAsSeries(c, true);
   if(CopyBuffer(g_hSotNatrATR, 0, 1, n, a) < n) return false;
   if(CopyClose(g_activeSymbol, InpSotTimeframe, 1, n, c) < n) return false;
   ...
   double pct = (double)less / (double)n * 100.0;
   return (pct >= InpSotNatrLowPct && pct <= InpSotNatrHighPct);
}

//+------------------------------------------------------------------+
//| H4 EMA21 slope confluence: slope sign agreement with direction    |
//+------------------------------------------------------------------+
bool SotConfPass(int dir)
{
   if(!InpSotUseConf) return true;
   if(g_hSotConf == INVALID_HANDLE) return true;
   double b[];
   ArraySetAsSeries(b, true);
   int need = InpSotConfSlopeBars + 2;
   if(CopyBuffer(g_hSotConf, 0, 1, need, b) < need) return true;
   return (dir == 1) ? (b[0] > b[InpSotConfSlopeBars])
                     : (b[0] < b[InpSotConfSlopeBars]);
}

// Mean |close-open| over the `len` bars BEFORE impulse bar m (m+1 .. m+len),
// matching fvg.py's avgBody (excludes the impulse bar itself).
double SotAvgBodyBefore(int m, int len)
{
   double sum = 0;
   for(int k = 1; k <= len; k++)
      sum += MathAbs(iClose(g_activeSymbol, InpSotTimeframe, m + k) -
                     iOpen(g_activeSymbol, InpSotTimeframe, m + k));
   return sum / len;
}

// Directional impulse RECENCY gate (impulse.py compute_impulse_recency): body=
// |c-o|; ref=sma(body,14); isImp = body >= ref*1.3; directional (dir>0 -> bull,
// dir<0 -> bear); 1-bar recognition lag + lookback 13 => at eval bar 1 the
// window is j in [2..14].
bool SotRecentImpulse(int dir)
{
   for(int j = 2; j <= 14; j++)
   {
      double cj = iClose(g_activeSymbol, InpSotTimeframe, j);
      double oj = iOpen(g_activeSymbol, InpSotTimeframe, j);
      double bodyJ = MathAbs(cj - oj);
      double sum = 0;
      for(int k = 0; k < 14; k++)
         sum += MathAbs(iClose(g_activeSymbol, InpSotTimeframe, j + k) -
                        iOpen(g_activeSymbol, InpSotTimeframe, j + k));
      double ref = sum / 14.0;
      if(ref <= 0) continue;
      if(bodyJ < ref * InpSotImpBodyMult) continue;
      if(dir > 0 && cj > oj) return true;
      if(dir < 0 && cj < oj) return true;
   }
   return false;
}
...
//+------------------------------------------------------------------+
//| B — Imbalance (FVG) pullback entries (long / short)               |
//+------------------------------------------------------------------+
void SotScanBLong(double atr)
{
   if(!SotRecentImpulse(1)) return;
   for(int m = 2; m <= InpSotFvgMaxAge; m++)
   {
      double impLow    = iLow(g_activeSymbol, InpSotTimeframe, m);
      double olderHigh = iHigh(g_activeSymbol, InpSotTimeframe, m + 2);
      if(impLow <= olderHigh) continue;

      double cM = iClose(g_activeSymbol, InpSotTimeframe, m);
      double oM = iOpen(g_activeSymbol, InpSotTimeframe, m);
      if(cM <= oM) continue;
      double avgB = SotAvgBodyBefore(m, InpSotAvgBodyLen);
      if(avgB <= 0 || MathAbs(cM - oM) < InpSotDispMult * avgB) continue;

      double zoneTop = impLow;
      double zoneBot = olderHigh;

      bool armed = false;
      for(int k = m; k >= 1; k--)
         if(iClose(g_activeSymbol, InpSotTimeframe, k) > zoneTop) { armed = true; break; }
      if(!armed) continue;

      if(InpSotFvgMitGuard)
         if(iClose(g_activeSymbol, InpSotTimeframe, 2) < zoneBot &&
            iClose(g_activeSymbol, InpSotTimeframe, 1) < zoneBot) continue;

      if(InpSotFvgTouch)
      {
         if(iLow(g_activeSymbol, InpSotTimeframe, 1) > zoneBot) continue;
      }
      else
      {
         if(iLow(g_activeSymbol, InpSotTimeframe, 1) > zoneBot &&
            iClose(g_activeSymbol, InpSotTimeframe, 1) > zoneBot) continue;
      }

      SotPlaceEntry(1, atr, "DX1_S_B_L");
      return;
   }
}

void SotScanBShort(double atr)
{
   if(!SotRecentImpulse(-1)) return;
   for(int m = 2; m <= InpSotFvgMaxAge; m++)
   {
      double impHigh  = iHigh(g_activeSymbol, InpSotTimeframe, m);
      double olderLow = iLow(g_activeSymbol, InpSotTimeframe, m + 2);
      if(impHigh >= olderLow) continue;
      ...
      double zoneTop = olderLow;
      double zoneBot = impHigh;

      bool armed = false;
      for(int k = m; k >= 1; k--)
         if(iClose(g_activeSymbol, InpSotTimeframe, k) < zoneBot) { armed = true; break; }
      if(!armed) continue;

      if(InpSotFvgMitGuard)
         if(iClose(g_activeSymbol, InpSotTimeframe, 2) > zoneTop &&
            iClose(g_activeSymbol, InpSotTimeframe, 1) > zoneTop) continue;

      if(InpSotFvgTouch)
      {
         if(iHigh(g_activeSymbol, InpSotTimeframe, 1) < zoneTop) continue;
      }
      else
      {
         if(iHigh(g_activeSymbol, InpSotTimeframe, 1) < zoneTop &&
            iClose(g_activeSymbol, InpSotTimeframe, 1) < zoneTop) continue;
      }

      SotPlaceEntry(-1, atr, "DX1_S_B_S");
      return;
   }
}

//+------------------------------------------------------------------+
//| D — Break & Retest entries (long / short)                         |
//+------------------------------------------------------------------+
void SotScanDLong(double atr)
{
   if(InpSotDImpFilter && !SotRecentImpulse(1)) return;
   double tol = InpSotDRetestTolATR * atr;

   for(int b = 2; b <= InpSotDMaxBarsAfterBreak + 1; b++)
   {
      int p = -1;
      for(int p_ = b + 1; p_ <= b + 40; p_++)
      {
         double hiP = iHigh(g_activeSymbol, InpSotTimeframe, p_);
         if(hiP > iHigh(g_activeSymbol, InpSotTimeframe, p_ + 1) &&
            hiP > iHigh(g_activeSymbol, InpSotTimeframe, p_ - 1))
         { p = p_; break; }
      }
      if(p < 0) continue;
      double level = iHigh(g_activeSymbol, InpSotTimeframe, p);

      if(iClose(g_activeSymbol, InpSotTimeframe, b) <= level) continue;
      if(iClose(g_activeSymbol, InpSotTimeframe, b + 1) > level) continue;

      if(iLow(g_activeSymbol, InpSotTimeframe, 1) <= level + tol)
      {
         SotPlaceEntry(1, atr, "DX1_S_D_L");
         return;
      }
   }
}

void SotScanDShort(double atr)
{
   if(InpSotDImpFilter && !SotRecentImpulse(-1)) return;
   double tol = InpSotDRetestTolATR * atr;
   ...
   for(int b = 2; b <= InpSotDMaxBarsAfterBreak + 1; b++)
   {
      ...
      for(int p_ = b + 1; p_ <= b + 40; p_++)
      {
         double loP = iLow(g_activeSymbol, InpSotTimeframe, p_);
         if(loP < iLow(g_activeSymbol, InpSotTimeframe, p_ + 1) &&
            loP < iLow(g_activeSymbol, InpSotTimeframe, p_ - 1))
         { p = p_; break; }
      }
      if(p < 0) continue;
      double level = iLow(g_activeSymbol, InpSotTimeframe, p);

      if(iClose(g_activeSymbol, InpSotTimeframe, b) >= level) continue;
      if(iClose(g_activeSymbol, InpSotTimeframe, b + 1) < level) continue;

      if(iHigh(g_activeSymbol, InpSotTimeframe, 1) >= level - tol)
      {
         SotPlaceEntry(-1, atr, "DX1_S_D_S");
         return;
      }
   }
}
...
//+------------------------------------------------------------------+
//| Module S entry: SL = ATR(6) x 1.7, TP fixed RR 1.0, hard true-RR  |
//+------------------------------------------------------------------+
bool SotPlaceEntry(int dir, double atr, string comment)
{
   long spread = SymbolInfoInteger(g_activeSymbol, SYMBOL_SPREAD);
   if(spread > InpMaxSpreadPts) return false;
   ...
   double entry = (dir == 1) ? SymbolInfoDouble(g_activeSymbol, SYMBOL_ASK)
                             : SymbolInfoDouble(g_activeSymbol, SYMBOL_BID);
   double risk = InpSotSL_ATRMult * atr;
   double sl = (dir == 1) ? entry - risk : entry + risk;
   double tp = (dir == 1) ? entry + risk * InpSotTP_RR : entry - risk * InpSotTP_RR;
   if(risk <= 0) return false;
   if((dir == 1 && sl >= entry) || (dir == -1 && sl <= entry)) return false;
   ...
   if(!ok)
   {
      Print("SOT_ENTRY_REJECTED | ", comment, " | ", GetLastError());
      return false;
   }
   ...
   g_lastTrailBar = 0;
   g_sotPosition = true;
   CaptureTradeMeta("SOT", comment, sl, tp);
   Print("TRADE_APPROVED | SOT | ", comment, " | ", (dir == 1) ? "BUY" : "SELL",
         " | entry=", DoubleToString(g_entryPrice, _Digits),
         " | sl=", DoubleToString(sl, _Digits),
         " | risk=", DoubleToString(risk, _Digits));
   return true;
...
//+------------------------------------------------------------------+
//| Module S engine (evaluated once per new InpSotTimeframe bar)      |
//+------------------------------------------------------------------+
void SotEngine()
{
   static datetime sotLastBar = 0;
   datetime bar = iTime(g_activeSymbol, InpSotTimeframe, 0);
   if(bar <= 0 || bar == sotLastBar) return;
   sotLastBar = bar;

   int gmtMin = ContGmtMinOfDay();
   if(!SotDayPass(SotGmtDOW())) return;
   if(!SotSessionPass(gmtMin)) return;

   double atr = SotGetATR();
   if(atr <= 0) return;
   if(!SotNatrPass()) return;

   int dir = SotTrendDir();
   if(dir == 0) return;
   if(!SotConfPass(dir)) return;

   // Router priority D BEFORE B (orchestrator.py module router: B&R > Imbalance).
   if(dir == 1)
   {
      if(InpSotUseD) SotScanDLong(atr);
      if(InpSotUseB && !g_inPosition) SotScanBLong(atr);
   }
   else if(InpSotAllowShort)
   {
      if(InpSotUseD) SotScanDShort(atr);
      if(InpSotUseB && !g_inPosition) SotScanBShort(atr);
   }
}
