...
input group "=== GEN-2 Exit (payoff) ==="
input bool   InpRunner  = false;             // runner exit: trail from +1.0R (no fixed 1R TP)
input double InpRunnerCapR = 3.0;            // runner outer TP cap in R (only when InpRunner)
input bool   InpPartial  = false;             // close 50% at +1.0R (bank half; requires InpRunner)
...
void TryEntry(double extLine)
{
   ...
   double tp = (InpRunner) ? NormalizeDouble(entry + risk * InpRunnerCapR, _Digits)
                           : NormalizeDouble(entry + risk * InpTP_RR, _Digits);
...
void ManagePosition()
{
   ...
   // partial 50% @ +1.0R (bank half; runner trails the remainder)
   if(InpPartial && r >= 1.0 && !g_partialDone && g_posDir == +1)
   {
      if(PositionSelectByTicket(openTk))
      {
         ...
         {
            g_partialDone = true;
            Print("VWAPX: partial 50% @ R=", DoubleToString(r, 2));
            return;
         }
      }
   }
   ...
   // runner exit: chandelier trail at 1R below peak once +1.0R reached (long-only)
   if(InpRunner && r >= 1.0 && g_posDir == +1)
   {
      if(PositionSelectByTicket(openTk))
      {
         double trailSL = NormalizeDouble(g_entryPrice + (g_mfeR - 1.0) * g_riskPts * _Point, _Digits);
         double curSL = PositionGetDouble(POSITION_SL);
         if(trailSL > curSL && trade.PositionModify(openTk, trailSL, PositionGetDouble(POSITION_TP)))
            Print("VWAPX: runner trail | SL=", DoubleToString(trailSL, _Digits), " | R=", DoubleToString(r, 2));
      }
   }
}
