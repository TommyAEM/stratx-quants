...
#property strict
#property description "DE40 VPPOC Gen2 - champion + evidence-directed entry gates (VAW/POC/DISP) + exit shaping (partial/BE/trail/session-close)"
...
input int    InpMagic        = 4406;          // Gen2 branch magic (unique per config; sets override)
input int    InpColdStartSec = 30;            // Cold-start delay after attach (sec)

input group "=== Gen2 Entry Gates (evidence-directed) ==="
input bool   InpGateVAW   = false;          // require VA width >= InpVAWMin (high band only)
input double InpVAWMin     = 1.03;           // min (VAH-VAL)/D1ATR14 to accept entry
input bool   InpGatePoc    = false;          // exclude near-POC entries (|f_poc_dist| < InpPocMin)
input double InpPocMin     = 0.076;          // min |entry-POC|/D1ATR14 to accept entry
input bool   InpGateDisp   = false;          // require f_disp <= InpDispMax (exclude high displacement)
input double InpDispMax    = 0.704;          // max 3-bar body / M15 ATR14

input group "=== Gen2 Exit Shaping (turned-loser repairs) ==="
input bool   InpPartial    = false;          // close 50% at +0.6R (runner keeps TP)
input bool   InpBE          = false;          // move SL to breakeven at +0.6R
input bool   InpTrail       = false;          // trail SL at 1R below peak after +1.0R
input bool   InpSessionClose = false;         // close open position at 16:59 GMT
...
// Gen2 exit-shaping state
bool     g_partialDone = false;              // partial close executed this position
bool     g_beMoved     = false;              // SL moved to breakeven
...
void TryEntry(int dir, string comment)
{
   ...
   ComputeForensics(entry);   // Gen2: entry forensics computed BEFORE gate + order

   if(InpGateVAW && g_f_va_width < InpVAWMin)
   {
      Print("VPPOC: GateVAW blocked | f_va_width=", DoubleToString(g_f_va_width, 3));
      return;
   }
   if(InpGatePoc && (g_f_poc_dist <= -8.0 || MathAbs(g_f_poc_dist) < InpPocMin))
   {
      Print("VPPOC: GatePoc blocked | f_poc_dist=", DoubleToString(g_f_poc_dist, 3));
      return;
   }
   if(InpGateDisp && g_f_disp > InpDispMax)
   {
      Print("VPPOC: GateDisp blocked | f_disp=", DoubleToString(g_f_disp, 3));
      return;
   }
...
void ManagePosition()
{
   ...
   // ---- Gen2 exit shaping (turned-loser repairs) ----
   if(InpSessionClose || InpPartial || InpBE || InpTrail)
   {
      if(!PositionSelectByTicket(openTk)) return;
      if(InpSessionClose)
      {
         MqlDateTime g2; GmtTime(TimeCurrent(), g2);
         if(g2.hour >= 16 && g2.min >= 59)
         {
            if(trade.PositionClose(openTk))
               Print("VPPOC: Gen2 session-close exit | R=", DoubleToString(r, 3));
            return;
         }
      }
      if(InpBE && r >= 0.6 && !g_beMoved)
      {
         if(trade.PositionModify(openTk, g_entryPrice, PositionGetDouble(POSITION_TP)))
         { g_beMoved = true; Print("VPPOC: Gen2 BE move | R=", DoubleToString(r, 2)); }
      }
      if(InpTrail && r >= 1.0)
      {
         double trailSL = (g_posDir == +1)
            ? g_entryPrice + (g_mfeR - 1.0) * g_riskPts * _Point
            : g_entryPrice - (g_mfeR - 1.0) * g_riskPts * _Point;
         trailSL = NormalizeDouble(trailSL, _Digits);
         double curSL = PositionGetDouble(POSITION_SL);
         bool better = (g_posDir == +1 && trailSL > curSL) || (g_posDir == -1 && trailSL < curSL);
         if(better && trade.PositionModify(openTk, trailSL, PositionGetDouble(POSITION_TP)))
            Print("VPPOC: Gen2 trail | SL=", DoubleToString(trailSL, _Digits));
      }
      if(InpPartial && r >= 0.6 && !g_partialDone)
      {
         ...
         double closeVol = (step > 0) ? MathFloor(vol * 0.5 / step) * step : 0.0;
         if(closeVol >= minL && trade.PositionClosePartial(openTk, closeVol))
         {
            g_partialDone = true;
            Print("VPPOC: Gen2 partial close ", DoubleToString(closeVol, 2), " @", DoubleToString(r, 2), "R");
         }
      }
   }
}
